...
void Vector<T>::autoIncreaseLength () {
    if (size_ >= capacity_ && capacity_ != 0) {
        T* temp_point = (T*) realloc ((void*) data_,
                                      capacity_ * INCREASE_FACTOR * sizeof (T));
...
template <class T>
inline void Vector<T>::autoCallDestructor (size_t num_object) {
    if (std::is_class<T>::value == true) {
            data_[num_object].~T();
    }
}
